...
def get_aws_costs_from_files():
    """Read AWS cost data from local JSON files"""
    try:
        aws_data = {
            "total": None,
            "service": None,
        ...
        # Path to AWS cost reports directory
        aws_dir = "aws-cost-reports"
        
        # Log available files for debugging
        all_aws_files = glob.glob(os.path.join(aws_dir, "*.json"))
        if not all_aws_files:
            st.warning(f"No JSON files found in {aws_dir} directory")
            # Try looking in current directory
            all_aws_files = glob.glob("*.json")
            if all_aws_files:
                st.info(f"Found JSON files in current directory instead: {all_aws_files}")
                aws_dir = "."
        
        # Find all relevant files with broader patterns
        service_files = glob.glob(os.path.join(aws_dir, "*SERVICE*.json")) + glob.glob(os.path.join(aws_dir, "*service*.json"))
        project_files = glob.glob(os.path.join(aws_dir, "*Project*.json")) + glob.glob(os.path.join(aws_dir, "*project*.json"))
        account_files = glob.glob(os.path.join(aws_dir, "*LINKED_ACCOUNT*.json")) + glob.glob(os.path.join(aws_dir, "*account*.json"))
        billing_cycle_files = glob.glob(os.path.join(aws_dir, "*billing_cycle_total*.json"))
        project_by_region_files = glob.glob(os.path.join(aws_dir, "*project_by_region*.json"))
        project_by_resource_files = glob.glob(os.path.join(aws_dir, "*project_by_resource*.json"))
        
        # Load SERVICE data (for total and service costs)
        if service_files:
            # Use the most recent file
            service_file = max(service_files, key=os.path.getmtime)
            ...
            aws_file = max(all_aws_files, key=os.path.getmtime)
        ...
        if project_files:
            # Use the most recent file
            project_file = max(project_files, key=os.path.getmtime)
            with open(project_file, 'r') as f:
                aws_data["project"] = json.load(f)
        
        # Load Account data (if available)
        if account_files:
            account_file = max(account_files, key=os.path.getmtime)
            with open(account_file, 'r') as f:
                aws_data["account"] = json.load(f)
        
        # Load billing cycle total
        if billing_cycle_files:
            billing_cycle_file = max(billing_cycle_files, key=os.path.getmtime)
            with open(billing_cycle_file, 'r') as f:
                aws_data["billing_cycle"] = json.load(f)
        
        # Load project by region data
        if project_by_region_files:
            project_by_region_file = max(project_by_region_files, key=os.path.getmtime)
            with open(project_by_region_file, 'r') as f:
                aws_data["project_by_region"] = json.load(f)
        
        # Load project by resource data
        if project_by_resource_files:
            project_by_resource_file = max(project_by_resource_files, key=os.path.getmtime)
            with open(project_by_resource_file, 'r') as f:
                aws_data["project_by_resource"] = json.load(f)
        
        return aws_data
    except Exception as e:
        st.error(f"Error reading AWS cost data files: {str(e)}")
        return None
...
def get_azure_costs_from_files():
    """Read Azure cost data from local JSON files"""
    try:
        # Path to Azure cost reports directory
        azure_dir = "azure-cost-reports"
        
        results = {}
        
        # Find all JSON files in the Azure directory
        azure_files = glob.glob(os.path.join(azure_dir, "*.json"))
        ...
        st.error(f"Error reading Azure cost data files: {str(e)}")
        return None
